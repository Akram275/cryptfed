...
class ThresholdBGVManager(BaseFHEManager):
    def __init__(self, security_level: str = 'standard', benchmark_manager: Any = None, threshold: int = 1):
        ...
        # Use same quantization scale as single-key BGV for consistency
    ...
    def generate_crypto_context(self) -> Any:
        ...
        logging.getLogger(__name__).info(f"Threshold BGV context generated. Slot count: {self.slot_count}, Plaintext Modulus: {p} ({np.round(math.log(p, 2))} bits)")
        return self.cc
