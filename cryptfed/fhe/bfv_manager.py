...
class BFVManager(BaseFHEManager):
    ...
    def generate_crypto_context_and_keys(self) -> None:
        ...
        p = 17180557313 # A large prime such that p-1 is divisible by ring dimension
        ...
        if self.benchmark_manager:
            self.benchmark_manager.log_event('FHE Setup (BFV)', 'Setup Time', duration)
        logging.getLogger(__name__).info(f"BFV context generated. Slot count: {self.slot_count}, Plaintext Modulus: {p} ({np.round(math.log(p, 2))} bits)")
